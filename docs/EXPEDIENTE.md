...
| 4 | **`TA` nombraba dos tableros distintos** (Operación semanal y el track de Casa Alysa). **ESCRITO 4-sep: Portero v47** — cada track con su código propio (`AL`, `TM`, `TC`) y `AL` ya concedido a las 4 cuentas que tenían `TA`, para que nadie pierda acceso. Falta publicar. | esperando 1 clic | Alejandro | La versión 47 implementada + la prueba de humo por rol |
| 5 | **`boards` vacío abría todo.** **ESCRITO 4-sep: Portero v47** — blanco = sin acceso, y el mismo criterio en la copia rápida del canje. Ninguna cuenta activa tiene `boards` en blanco (verificado), así que al publicar nadie pierde nada. Falta publicar. | esperando 1 clic | Alejandro | La versión 47 implementada |
